  ...
  return (
    <div ref={ref} className="relative h-full w-full">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={clsx(
          'group flex h-full min-h-[4rem] w-full items-center justify-center gap-2 transition',
          'hover:bg-white/[0.04]',
      ...
      >
        {displayValue ? (
          ...
          <span className="font-display text-3xl leading-none text-[var(--fg-mute)] opacity-40 transition group-hover:opacity-100">
            +
          </span>
        )}
        {isTie && (
      ...
      {open && (
        <div className="absolute left-1/2 top-full z-30 mt-1 -translate-x-1/2 border border-[var(--rule-strong)] bg-[var(--bg-elev)] shadow-[0_18px_36px_-8px_rgba(0,0,0,0.7)]">
          <div className="flex">
            {canTie && (
              <button
                type="button"
                onClick={commitTie}
                aria-label={`Set ${row.label} as a tie`}
                className={clsx(
                  'flex h-12 w-14 items-center justify-center font-display text-base uppercase tracking-[0.2em] transition hover:bg-[var(--accent)] hover:text-[var(--bg)]',
                  isTie ? 'bg-[var(--accent)] text-[var(--bg)]' : 'text-[var(--fg)]',
                )}
            ...
            {(canTie ? CATEGORY_MARGINS : POSE_MARGINS).map((m, i) => {
              ...
              return (
                <button
                  ...
                  aria-label={`Set ${row.label} margin ${m} for side ${side}`}
                  className={clsx(
                    'flex h-12 w-12 items-center justify-center font-display text-2xl text-[var(--fg)] transition hover:bg-[var(--accent)] hover:text-[var(--bg)]',
                    (canTie || i > 0) && 'border-l border-white/20',
                    selected && 'bg-[var(--accent)] text-[var(--bg)]',
                  )}
                >
                  {m}
                </button>
            ...
            <button
              type="button"
              onClick={clear}
              aria-label={`Clear score for ${row.label}`}
